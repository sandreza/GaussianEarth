...
for (i, field) in enumerate(fields)
    ...
    ax = Axis(fig[i, 1]; title = field, xlabel = "Mode", ylabel = "Kurtosis")
    scatter!(ax, κ[1:modes], color = :blue)
    ylims!(ax, (-1,1))
    ax = Axis(fig[i, 2]; title = field, xlabel = "Mode", ylabel = "Skewness")
    scatter!(ax, ρ[1:modes], color = :red)
    ylims!(ax, (-1,1))
end
display(fig)
save("all_fields_skew_kurt.png", fig)