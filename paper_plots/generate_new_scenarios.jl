...
scenario_1[1] = init_temp
scenario_2[1] = init_temp
scenario_1[end] = end_temp
scenario_2[end] = end_temp
...
k2 = 0.06  # Slightly faster speed for scenario 2 (to create a difference)

n= length(time_future) - 17
init_temp = scenario_1[16]
for (t, i) in enumerate(17:length(time_future)-1)
    scenario_1[i] = init_temp + (end_temp - init_temp) * (log(1 + k1 * t) / log(1 + k1 * n))
    scenario_2[i] = init_temp + (end_temp - init_temp) * (log(1 + k2 * t) / log(1 + k2 * n))
end
...
lines!(ax, time_future, ssp2, label="SSP2-4.5", color=:purple)
