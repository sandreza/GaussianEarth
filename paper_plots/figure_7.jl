for scenario in ["ssp245", "ssp585"]
    ...
    for i in eachindex(observables)
        ...
        if use_bundle && has_ground_truth_bundle(bundle_path)
            key = i == 1 ? "land" : "ocean"
            tas_hist = f7_samples("tas", month, key, "historical")
        else
        ...
        if use_bundle && has_ground_truth_bundle(bundle_path)
            key = i == 1 ? "land" : "ocean"
            hurs_hist = f7_samples("hurs", month, key, "historical")
        else
        ...
        if use_bundle && has_ground_truth_bundle(bundle_path)
            key = i == 1 ? "land" : "ocean"
            tas_hist = f7_samples("tas", month, key, "ssp245")
            hurs_hist = f7_samples("hurs", month, key, "ssp245")
        else
    ...
    elseif scenario == "ssp585"
        save(figure_directory * "figure_D2_climate_change_shifts_hurs_tas_with_data_jan_july_land_sea_"*scenario*".png", fig)
        @info "Generated Figure D2."
    end
end