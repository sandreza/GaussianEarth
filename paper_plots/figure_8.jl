for scenario in ["ssp245", "ssp585"]
    ...
    for i in eachindex(observables)
        ...
        if use_bundle && has_ground_truth_bundle(bundle_path)
            ...
            tas_hist = f8_samples("tas", month, key, "historical")
        else
        ...
        if use_bundle && has_ground_truth_bundle(bundle_path)
            key = i == 1 ? "loc_157_46" : "loc_19_87"
            hurs_hist = f8_samples("hurs", month, key, "historical")
        else
        ...
        if use_bundle && has_ground_truth_bundle(bundle_path)
            key = i == 1 ? "loc_157_46" : "loc_19_87"
            tas_hist = f8_samples("tas", month, key, "ssp245")
            hurs_hist = f8_samples("hurs", month, key, "ssp245")
        else
    ...
    # display(fig)
    if scenario == "ssp245"
        save(figure_directory * "Figure_8_climate_change_shifts_hurs_tas_with_data_jan_july_points_"*scenario*".png", fig)
        @info "Generated Figure 8."
    elseif scenario == "ssp585"
        save(figure_directory * "Figure_D3_climate_change_shifts_hurs_tas_with_data_jan_july_points_"*scenario*".png", fig)
        @info "Generated Figure D3."
    end
end